...
public class BasicAssociationModelProvider implements Provider<AssociationModel> {
    ...
    @Inject
    public BasicAssociationModelProvider(@Transient final DataAccessObject dao) {
        this.dao = dao;
    }
    ...
    public AssociationModel get() {
        ...
        for (final Long2ObjectMap.Entry<LongSortedSet> xEntry : itemUsers.long2ObjectEntrySet()) {
            ...
            for (final Long2ObjectMap.Entry<LongSortedSet> yEntry : itemUsers.long2ObjectEntrySet()) {
                ...
                itemScores.put(yId, (double) LongUtils.intersectSize(xUsers, yUsers) / (double) xUsers.size());
            }
